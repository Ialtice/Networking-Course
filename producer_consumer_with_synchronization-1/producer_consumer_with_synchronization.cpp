...
pthread_cond_t has_order = PTHREAD_COND_INITIALIZER;
pthread_cond_t has_room_for_order = PTHREAD_COND_INITIALIZER;
...
void* takeOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
        ...
        if(num_new_orders == N)
        	pthread_cond_wait(&has_room_for_order,&data_mutex);
...
void* processOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
        ...
        if(num_new_orders == 0)
        	pthread_cond_wait(&has_order, &data_mutex);
        	
        o_num = new_orders[index].order_num;
	    item = new_orders[index++].item_num;
        --num_new_orders;
        
		pthread_cond_signal(&has_room_for_order);
